      ...
      {fields.map((f) => (
        <div key={f.label}>
          <label className="text-xs font-medium mb-1 block" style={{ color: colors.muted }}>
            {f.label}{f.optional && <span className="ml-1 opacity-60">(if available)</span>}
          </label>
          <input type="number" step="0.01" value={f.value} onChange={(e) => f.set(e.target.value)}
            placeholder="0.00"
            className="w-full px-4 py-2.5 rounded-xl text-sm border bg-transparent focus:outline-none"
            style={inputStyle(colors)} />
          <p className="text-xs mt-0.5" style={{ color: `${colors.muted}70` }}>{f.hint}</p>
        </div>
      ))}
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div>
        ...
        <input type="number" value={waist} onChange={(e) => setWaist(e.target.value)}
          ...
          className="w-full px-3 py-2.5 rounded-xl text-sm border bg-transparent focus:outline-none"
          style={inputStyle(colors)} />
        <p className="text-xs mt-1" style={{ color: `${colors.muted}70` }}>Tape around bare abdomen at navel height. Don't pull tight.</p>
      </div>
      <button disabled={!ready}
        onClick={() => {
...
const GUIDANCE: Record<string, { title: string; body: React.ReactNode }> = {
  ...
  bas_fasting_confirmed: {
    ...
    body: (
      <>
        <p>Your glucose and lipid readings are only meaningful fasted. Eating raises blood sugar and triglycerides for hours, skewing your BAS.</p>
    ...
    body: (
      <>
        <ol className="space-y-2 list-decimal list-inside text-xs">
          ...
          <li>Wait for the reading and note the number</li>
        </ol>
        <div className="mt-4 p-3 rounded-xl text-xs" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
    ...
    body: (
      <>
        <ol className="space-y-2 list-decimal list-inside text-xs">
          ...
          <li>Prick your finger again (or reuse the same drop within 30 s)</li>
          <li>Apply blood to the strip</li>
          <li>Do not move the meter — wait 3 minutes</li>
          <li>The meter shows TG, HDL, LDL in sequence — write all down before entering</li>
        </ol>
        <div className="mt-4 p-3 rounded-xl text-xs" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
          If your meter also shows Total Cholesterol (TC), enter it too.
        </div>
      </>
    ),
  },
  bas_anthropometrics: {
    ...
    body: (
      <>
        <p className="font-semibold text-sm mb-2">How to measure your waist</p>
        <p className="text-xs">Stand relaxed. Wrap a tape measure around your bare abdomen at navel height. Breathe out naturally — don't hold your breath or pull the tape tight.</p>
        <p className="mt-3 text-xs">Height and weight can be from a recent measurement if you don't have scales handy.</p>
        <div className="mt-3 p-3 rounded-xl text-xs" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
          <strong>Year of birth</strong> — used to compute your age-adjusted BAS score. Entering your birth year instead of raw age gives the scoring engine the precision it needs.
        </div>
      </>
    ),
  },
  bas_complete: {
    ...
    body: (
      <>
        <p>Your BAS combines fasting glucose, lipid panel, and visceral fat estimate from your measurements.</p>
        <div className="mt-3 space-y-2 text-xs">
          <div><strong>BAS lower than your age</strong> — metabolically younger</div>
          <div><strong>BAS higher than your age</strong> — room to improve; MeO will guide you</div>
        </div>
        <p className="mt-3 text-xs">Your data has been submitted automatically. Check the Analysis tab for your score.</p>
      </>
    ),
  },
};
